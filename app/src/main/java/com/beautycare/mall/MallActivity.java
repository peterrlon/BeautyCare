...
//        //这个方法是在onPreExecute后执行的，params参数的类型是第一个Void，返回的类型是第三个ArrayList<MallData>，返回的参数会传到onPostExecute
//
//            try {
//                AVObject mallObject;
//                AVFile logoObject;
...
//
//                        MallData tmpData = new MallData();//封装好的数据类型
//                        mallObject = result.get(i);//获取第i个AVObject
//                        /*通过key获取AVObject（即数据库）相应的列的内容并调用封装数据类的方法添加数据*/
//                        tmpData.setMallName(mallObject.getString("mall_name"));
//                        /*同理，这里因为每个数据库表的列名都不同，
//                        我这里直接就写上key名了，如果这个要封装的话不知道怎么封装，所以再不行的话每人写自己的asynctask
//                         */
//                        tmpData.setMallContent(mallObject.getString("content"));
//
